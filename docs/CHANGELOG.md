...
- [修复] Agent 决策仪表盘系统 prompt（`AGENT_SYSTEM_PROMPT` / `LEGACY_DEFAULT_AGENT_SYSTEM_PROMPT`）在 `report_language` 为 `en`/`ko` 时整段切换为英文模板（工作流程、规则、JSON schema 示例、评分标准、核心原则、稳定性约束全部英文，schema 示例值为 `Company name` / `Strongly Bullish` 等），`en`/`ko` 均移除此前 `_build_language_section` 返回中文指令的问题，`zh` 保持原中文模板不变；修复 `REPORT_LANGUAGE=en` 时同批次中部分股票通知正文仍输出中文的模型口径漂移，并新增英文模板的 phase_decision 契约与"无中文"回归测试。
...
- [新功能] 将"公司报告"（叙述分析块）与"公司报告数据表"在通知中的展示拆成两个独立开关：新增 `INCLUDE_COMPANY_REPORTS_IN_NOTIFICATION`（控制 📑 叙述块）与 `INCLUDE_COMPANY_REPORTS_TABLE_IN_NOTIFICATION`（控制新增的 📊 数据表块），二者默认 `true`（保持"开启公司报告即出现在通知"的原行为，可分别关闭）；数据表块此前仅出现在 Web/API，现在也按开关追加到既有叙述块之后。
...
- [新增] 同时按本地日期轮转 `stock_analysis_YYYYMMDD.log` 常规日志与 `stock_analysis_debug_YYYYMMDD.log` 调试日志：系统日期变化（跨天）时自动创建新一天的日志文件并将后续内容写入新文件，长驻进程（如服务端）也会在跨天时自动切换；常规日志默认保留最近 5 个日期文件、调试日志默认保留最近 3 个，更早文件自动清理（替代原先仅按大小轮转、进程不重启会一直写同一天文件的 `RotatingFileHandler`）。
  <!-- 新条目格式：- [类型] 描述（类型取值：新功能/改进/修复/文档/测试/chore）-->
  <!-- 每条独立一行追加到本段末尾，无需分类标题，合并时冲突最小 -->
...
- [改进] `NOTIFICATION_SHORT=true` 时通知中 📊 数据透视（Data View）只保留均线排列一行（完整保留排列描述与趋势强度），省略价格指标表与量能分析，缩短通知长度；Web/API 端的展示保持不变。
  <!-- 新条目格式：- [类型] 描述（类型取值：新功能/改进/修复/文档/测试/chore）-->
  <!-- 每条独立一行追加到本段末尾，无需分类标题，合并时冲突最小 -->
