...
class CompanyReportsAnalyzer:
    ...
    def _build_prompt_en(
        ...
        return (
            ...
            "First in max 4 sentences give some remarks about the reports, that you as warren buffet have found that are noticable when you'd invest in the stock. \n"
            "Optionally, and ONLY if valuable: you may add a brief qualitative note comparing this company to a well-known direct competitor. Do NOT invent or assert \n"
            "specific peer numbers (margins, growth, valuation) — the competitor's financials are not in your input data. You may only reference a peer's approximate \n"
            "relative position if you are highly confident of it from widely-known public knowledge; otherwise state that a grounded peer comparison would require pulling \n"
            "the competitor's filings. Keep this to 1–2 sentences and clearly frame it as directional, not precise. and also start the sentence with -- compared to competetitors -- \n"
            "Second, answer previous questions in bullet points in order. e.g. \n"
            "1. Business Quality & Moat Signals (warren buffet's conclusion) \n"
            "2. Return on Capital => (warren buffet's conclusion)\n"
            "continue for the remaining questions, keep the format in bullet points. \n"

            "Some formatting requirements: \n"
            "- Keep each bullet to one tight line — plain ASCII, no LaTeX escapes."
