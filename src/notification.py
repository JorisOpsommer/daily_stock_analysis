...
def _format_strategy_skill_items(items: Any, report_language: str = "zh") -> str:
    ...
    for item in items:
        ...
        if not skill_id:
            continue
        suffix = (
            f"/{localize_strategy_signal(signal, report_language)}" if signal else ""
        )
        if isinstance(confidence, (int, float)):
            suffix += f"/{confidence:.0%}"
        formatted.append(
            f"{localize_strategy_skill(skill_id, report_language)}{suffix}"
        )
    ...
    for conflict in (strategy_synthesis.get("conflicts") or [])[:3]:
        if isinstance(conflict, dict) and conflict.get("conflict_type"):
            ...
            participant_text = "、".join(
                localize_strategy_skill(participant, report_language)
...
class NotificationService(
    ...
    def _get_display_name(
        self, result: AnalysisResult, language: str | None = None
    ) -> str:
    ...
    def generate_daily_report(
        self, results: list[AnalysisResult], report_date: str | None = None
    ) -> str:
        """
    ...
    def _get_signal_level(self, result: AnalysisResult) -> tuple:
        ...
        _, emoji, signal_tag = get_signal_level(
            signal_advice
            or self._get_display_operation_advice(result, report_language),
    ...
    def generate_dashboard_report(
        ...
        if not self._report_summary_only:
            for result in sorted_results:
                ...
                # ========== 信号归因分析 ==========
                signal_attr = (
                    dashboard.get("signal_attribution", {}) if dashboard else {}
                )
                if signal_attribution_has_content(signal_attr):
                    ...
                    if weight_items:
                        report_lines.append(
                            f"**{labels['attribution_weights_label']}**:"
                        )
                        weight_labels = {
                            "technical_indicators": (
                                "📈",
                                labels["technical_indicators_label"],
                            ),
                            "news_sentiment": ("📰", labels["news_sentiment_label"]),
                            "fundamentals": ("📊", labels["fundamentals_label"]),
                            "market_conditions": (
                                "🌐",
                                labels["market_conditions_label"],
                            ),
                        }
                    ...
                    if signal_attr.get("strongest_bullish_signal"):
                        report_lines.append(
                            f"**🐂 {labels['strongest_bullish_signal_label']}**: {signal_attr['strongest_bullish_signal']}"
                        )
                    if signal_attr.get("strongest_bearish_signal"):
                        report_lines.append(
                            f"**🐻 {labels['strongest_bearish_signal_label']}**: {signal_attr['strongest_bearish_signal']}"
                        )
                    report_lines.append("")
                ...
                _append_strategy_synthesis_block(
                    report_lines, strategy_synthesis, labels, report_language
                )
        ...
        if self._report_summary_only:
            ...
            for result in sorted_results:
                ...
                if strategy_synthesis:
                    ...
                    if invalid_count:
                        lines.append(
                            labels.get("strategy_invalid_opinions_label", "").format(
                                count=invalid_count
                            )
                        )
                    summary = localize_strategy_synthesis_summary(
                        strategy_synthesis, report_language
                    )
                    if summary:
        ...
        if signal_attribution_has_content(signal_attr):
            ...
            if weight_items:
                ...
                weight_labels = {
                    "technical_indicators": (
                        "📈",
                        labels.get("technical_indicators_label", "技术指标"),
                    ),
                    "news_sentiment": (
                        "📰",
                        labels.get("news_sentiment_label", "新闻舆情"),
                    ),
                    "fundamentals": ("📊", labels.get("fundamentals_label", "基本面")),
                    "market_conditions": (
                        "🌐",
                        labels.get("market_conditions_label", "市场环境"),
                    ),
                }
                for key, value in weight_items:
                    icon, label = weight_labels[key]
            ...
            if bullish:
                lines.append(
                    f"**🐂 {labels.get('strongest_bullish_signal_label', '最强看多信号')}**: {bullish}"
                )
            if bearish:
                lines.append(
                    f"**🐻 {labels.get('strongest_bearish_signal_label', '最强看空信号')}**: {bearish}"
                )
            lines.append("")
    ...
    def _get_fundamental_blocks(self, result: AnalysisResult) -> dict[str, Any]:
        ...
        concept_block = (
            ctx.get("concept_boards")
            if isinstance(ctx.get("concept_boards"), dict)
            else {}
        )
        ...
        concept_data = (
            concept_block.get("data")
            if isinstance(concept_block.get("data"), dict)
            else concept_block
        )
        if not isinstance(concept_data, dict):
            concept_data = {}
        concept_top = (
            concept_data.get("top") if isinstance(concept_data.get("top"), list) else []
        )
        concept_bottom = (
            concept_data.get("bottom")
            if isinstance(concept_data.get("bottom"), list)
            else []
        )
        ...
        institution_block = (
            ctx.get("institution") if isinstance(ctx.get("institution"), dict) else {}
        )
        institution_data = (
            institution_block.get("data")
            if isinstance(institution_block.get("data"), dict)
            else {}
        )
    ...
    def _append_company_reports_block(
        self,
        lines: list[str],
        result: AnalysisResult,
        labels: dict[str, str],
    ...
    def _append_related_boards(
        ...
        def add_signals(
            target: dict[str, tuple[str, float]], rows: Any, label: str
        ) -> None:
            for item in rows or []:
                if not isinstance(item, dict):
                    continue
        ...
        add_signals(
            sector_signals, blocks.get("sector_top"), labels["leading_board_label"]
        )
        add_signals(
            sector_signals, blocks.get("sector_bottom"), labels["lagging_board_label"]
        )
        add_signals(
            concept_signals, blocks.get("concept_top"), labels["leading_board_label"]
        )
        add_signals(
            concept_signals, blocks.get("concept_bottom"), labels["lagging_board_label"]
        )
        ...
        def resolve_board_type(name: str, board_type: str) -> str:
            ...
            if any(
                ...
                for marker in ("概念", "题材", "concept", "theme")
            ):
                return "concept"
            if any(
                marker in normalized_name for marker in ("行业", "industry", "sector")
            ):
                return "sector"
        ...
        for raw in belong_boards[:5]:
            ...
            prepared.append(
                (name, board_type_label(board_group), status_text, change_pct)
            )
        ...
        lines.append("")
        has_signal = any(
            status is not None and change_pct is not None
            for _, _, status, change_pct in prepared
        )
        if has_signal:
